...
class MangagoExtension implements MangagoImplementation {
  ...
  async getChapterDetails(chapter: Chapter): Promise<ChapterDetails> {
    ...
    // reader, so any stored URL that is not a proper
    // /read-manga/<slug>/<chapter> reader page would 404 forever: legacy
    // numeric /chapter/<mid>/<cid>/ URLs, and prefix-less /uu/<chapter>/pg-N/
    // paths from older doubled-host repairs. Re-resolve those from the current
    // manga chapter list before fetching images.
  ...
  private async resolveReadMangaChapterUrl(chapter: Chapter): Promise<string | undefined> {
    ...
    for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
      try {
        ...
        // No read-manga URLs this round: retry for a fresh mobile chapter list.
        ...
        const match =
          ...
          // chapNum === 0 is the "unnumbered" sentinel. Do not match all Extras,
          // Oneshots, Afterwords, etc. by number alone.
