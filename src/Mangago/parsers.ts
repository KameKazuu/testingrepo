...
// Groups whose names appear in inconsistent forms on the site; matching them
// explicitly keeps one group from splitting into several version strings.
const KNOWN_GROUPS = [
  {
    title: "Official",
    patterns: [/official/i],
...
// Uploader accounts that post official releases without saying so in the title.
...
function normalizeGroup(raw: string): string {
  ...
  return detectGroupFromTitle(text) || text;
}
