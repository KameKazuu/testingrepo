...
/** Pagination cursor for `PagedResults`. */
export interface Metadata extends JSONObject {
  page: number;
}

/**
 * Advanced-search selections. The API filters by numeric id, so every value
 * here is a stringified id understood by `q_type` / `q_status` / `q_tags`.
 */
